...
TV::TV(GameObject& associated, const std::string& file, vector<string>& actions, GameObject& mouse, const Vec2& scale)
...
bool TV::Is(const std::string& type){
  if (type == "TV")
    return true;
  return false;
}

void TV::NotifyCollision(GameObject &other) {
  if (sprite == nullptr)
  {
    Sprite *object_sprite = new Sprite(associated, "assets/img/tv.png");
